...
async function canManageTable(userId, role, table) {
    ...
    if (table.group_id) {
        const group = await get('SELECT * FROM groups WHERE (id = ? OR server_id = ?) AND is_deleted = 0', [table.group_id, table.group_id]);
        if (group && (group.owner_user_id === userId || group.created_by === userId)) return true;
        const membership = await get('SELECT * FROM group_members WHERE user_id = ? AND (group_id = ? OR group_id = ?)', [userId, table.group_id, group ? group.id : table.group_id]);
