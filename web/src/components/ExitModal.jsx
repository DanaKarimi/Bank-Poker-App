...
const ExitModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Failed to submit exit');
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between pb-3 mb-4 border-b border-[#d4af37]/30">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h3 className="text-xs sm:text-sm font-bold text-[#f5f5dc] uppercase tracking-[2px] truncate">
              {selectedPlayer
                ? `RECORD EXIT: ${(selectedPlayer.name || selectedPlayer.username).toUpperCase()}`
                : effectivePlayerName
                ? `RECORD EXIT: ${effectivePlayerName.toUpperCase()}`
                : 'RECORD EXIT'}
            </h3>
          </div>
          <button
          ...
          <div className="mb-4 space-y-2">
            ...
            {isNewPlayer && (
              <div className="p-2 bg-yellow-950/40 border border-yellow-500/40 rounded-lg text-[11px] text-yellow-300 flex items-center gap-1.5">
                ...
                <span>New player "{typedName.trim()}" will auto-seat & record exit.</span>
              </div>
            )}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            ...
            >
              ...
              <span>{isSubmitting ? 'SUBMITTING...' : 'CONFIRM EXIT'}</span>
            </button>
          </div>
        </form>
      </div>
